...
c.execute('''CREATE TABLE IF NOT EXISTS edithistory 
          (event text, edit_count, update_id text, author_name text, author_id text,  
           update_time, gengzheng_time, chuangjian_time, collect_time)''') #unique最后改一下
# 创建表单，保存事件下面的百科内部链接
...
#编辑历史表单
    ...
    for num in range(1,editpage_nos+1):
        ...
        for version in versions[1:]:
            ...
            #print('弹窗关闭啦！')
            #写入sql
            version_values = (entryname, editcount, hashid, contributor_name, contributor_id, 
                              submit_time, gengzheng_time, chuangjian_time,
                              datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
            #print('本次编辑历史准备写入sql')
