...
# -*- coding: utf-8 -*-
"""
Created on Sat Jan 14 19:19:31 2023
...
df = df.drop(['Unnamed: 0.1.1','Unnamed: 0.1'], axis=1) #删除之前的多余索引
...
driver.quit()
df.to_csv('citation+原始链接补15s.csv',index=False)

conn = sqlite.connect('citation+原始链接补15s.sqlite')
df.to_sql('citation+origi15s', conn, index=False)
conn.close()
