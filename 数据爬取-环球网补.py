...
# -*- coding: utf-8 -*-
"""
Created on Fri Mar 24 22:13:38 2023
...
# -*- coding: utf-8 -*-
"""
Created on Wed Mar 22 21:49:06 2023
...
driver = uc.Chrome()
#driver.get('https://china.huanqiu.com/article/3xVhnCmJBMz')

#os.chdir('/Users/zhangsiqi/Desktop/毕业论文代码mini/专门输出数据表/0124补充卫健委等时间')
df = pd.read_excel('/Users/zhangsiqi/Documents/毕业论文数据/专门输出数据表/0322/nhc.xlsx')
    ...
    if '环球' in maindo:
        if pd.notna(url) and pd.isna(row['source_time']) and pd.notna(row['cite_time']) and pd.isna(row['timestamp_x']) and pd.isna(row['org']) and pd.isna(row['htmltimestamp']) and pd.isna(row['handtime']):
            try:
                ...
                timestamp = get_elements(driver, By.CSS_Selector, 'div > div.fix-menu > div.date')
                ...
                df.at[index,'timestamp_x'] = ti.groupdict()['time']
            
            # filepath = '/Users/zhangsiqi/Documents/毕业论文数据/卫健委网站'
...
#写入csv & sql
os.chdir('/Users/zhangsiqi/Documents/毕业论文数据/专门输出数据表/0322')

df.to_csv("citation+news-huanq.csv", index=True)
